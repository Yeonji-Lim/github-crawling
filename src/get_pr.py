...
def get_repo_prs_last_day(r: Repository.Repository):
    print('===== start : get_repo_prs_last_day =====')
    return [p for p in r.get_pulls(state="closed") if p.user.name != "Deleted User" and p.closed_at >= last_day]