...
a_week_ago = yesterday - timedelta(days=7)
start_time = a_week_ago.strftime("%Y-%m-%d 00:00:00")
...
for member in members:
    print("===== start to get {}'s authored, assigned, reviewed weekly counts =====".format(
        member[1]))
    ...
    insert_daily_content_count_sql = "INSERT INTO `weekly_content_counts` (`member_id`, `authored_count`, `assigned_count`, `reviewed_count`, `aggregation_start_date`, `aggregation_end_date`) VALUES (%s, %s, %s, %s, %s, %s)"
    c.execute(insert_daily_content_count_sql, (member[0], autored_content_count[0], assigned_content_count[0],
              reviewed_content_count[0], a_week_ago.strftime("%Y-%m-%d"), yesterday.strftime("%Y-%m-%d")))
    conn.commit()
    print("===== result to get {}'s authored, assigned, reviewed weekly counts: {}, {}, {} =====".format(
